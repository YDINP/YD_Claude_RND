...
#### F-2: PreloadScene 에셋 로딩 검증 `[DONE]`
**검증 항목**:
- 모든 필요 에셋 프리로드 완료 여부
- 로딩 진행바 정확도
- 에셋 로딩 실패 시 에러 핸들링
...
#### F-3: MainMenuScene 데이터 바인딩 검증 `[DONE]`
**검증 항목**:
...
#### F-5: HeroListScene 표시 로직 검증 `[DONE]`
**검증 항목**:
- 보유 캐릭터 목록 정확히 표시
- 정렬 기능: 등급순, 레벨순, 전투력순
- 필터 기능: 교단별, 성격별, 역할별
...
#### F-6: HeroDetailScene 육성 시스템 검증 `[DONE]`
**검증 항목**:
...
#### F-7: StageSelectScene 전체 로직 검증 `[DONE]`
**검증 항목**:
...
#### F-11: GachaScene ↔ GachaSystem 상호작용 완전 검증 `[DONE]`
**검증 체인**:
```

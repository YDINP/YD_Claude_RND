...
> **2026-09-03 현재 충족 여부**: 14개 중 7개(DoD-1·2·3·4·6·7·10) 충족, 3개(DoD-5·8·9 중 9는 충족, 5·8은 부분) 부분, 4개(DoD-11·12·13·14) 미충족 또는 부분. 상세 근거는 아래 표 "현재 충족 여부" 열 참고.

| # | 기준 | 검증 방법 | 현재 충족 여부 (2026-09-03) |
|---|------|----------|------------------------------|
| DoD-1 | **빌드 체인 그린**: `tsc --noEmit` 0 에러, `npm run build` 성공, ESLint 0 에러 | CI(cy.yml) 통과 | ✅ 충족 — tsc 0 에러, build 성공 (실측). ESLint는 이번 세션에서 별도 재실행 미확인 |
| DoD-2 | **유닛테스트 전량 그린**: 수집 ≥ 685개, 실패 0. 스위트 로드 실패 0 | `npx vitest run` | ✅ 충족 — 58파일 1476개 전량 통과, 로드 실패 0 |
| DoD-3 | **코어 루프 E2E 완주**: 가챠 → 기본영웅 각성 → 기관 선택 전직 → 파티 편성 → 전투 → 보상/저장 → 재접속 복원, 전 체인 수동+E2E 검증 | Playwright 시나리오 | ✅ 충족 — `test:e2e:story`(114/114)가 온보딩→각성 컷씬→스킵 패리티→전투를 커버, `returning-storylog-smoke`(34/34)가 재접속 복원을 커버 |
| DoD-4 | **단일 플레이 콘텐츠 3축**: 모험 5챕터 25스테이지 + 무한의 탑 100층 + 이벤트 던전이 실제 플레이 가능하고 보상이 저장됨 | 스테이지 순회 E2E | ✅ 충족 — 탑 100층(TOWER-02), 스테이지 적 데이터 공백(BLK-01) 해소, 이벤트 던전은 기존 완료 확정 항목 |
| DoD-5 | **주간 콘텐츠 1종 이상**: 레이드 또는 탑 시즌 중 1개가 주간 리셋과 함께 동작 | 수동 플레이 + 타이머 테스트 | 🔶 부분 — RaidSystem(RAID-01/02) 주간 리셋·기여도 보상 로직은 구현·테스트됨(≥20건). 다중 유저 실시간 데미지 집계(R-07의 Realtime/폴링 설계)는 검증되지 않음 |
| DoD-6 | **수집 완성도**: 기본영웅 10명 전원 각성 가능, 24종 전직 영웅 획득/육성/편성 가능, 도감에서 진행률 표시 | 데이터↔UI 대조 | ✅ 충족 — CollectionSystem/CollectionPopup(COLL-01/02) 완료 |
| DoD-7 | **전투 깊이 확보**: Mood 상성 ±20% + 교단 시너지 + 최소 공세/제어/지속/균형 4그룹 각 1기관의 시그니처 메커니즘이 전투에 반영 | 전투 시뮬레이션 유닛테스트 | ✅ 충족(최소 기준) — `CultMechanicsSystem`이 4기관·토큰 20종 구현, `battle-cult-live.mjs`(23/23)로 실동작 검증. 나머지 8기관·36토큰은 DoD 최소 기준 밖의 확장 범위 |
| DoD-8 | **소셜 3종**: PvP 랭킹 ✅ / 길드 ✅ / 친구(대여+포인트) 동작, Supabase 장애 시 오프라인 폴백 | 네트워크 차단 테스트 | 🔶 부분 — 친구 시스템(FRIEND-01/02) 구현 완료로 3종 모두 존재하나, PvP 시뮬레이션 경로에 교단 메커니즘(Holy Thorns 등)이 반영되지 않아 일반 전투와 결과가 어긋날 수 있음 |
| DoD-9 | **경제 밸런스 검증**: 가챠 10,000회 시뮬레이션(등급 편차 ±0.5%), 재화 일일 수급/소모 시뮬레이션 리포트 존재 | 시뮬 스크립트 산출물 | ✅ 충족 — BAL-01 09-02 완료 |
| DoD-10 | **아트 완성**: 전 플레이 가능 캐릭터(34+)에 최종 포트레이트 적용, 플레이스홀더/CC0 임시본 제거 또는 수용 결정 문서화 | 에셋 감사 | ✅ 충족 — MVP 로스터(기본영웅 10+전직영웅 24=34) 전신·포트레이트 34장(hero_005~038) 확보. 레거시 char_1~4(hero_001~004)는 MVP 외 플레이스홀더로 수용 |
| DoD-11 | **사운드**: BGM 3종 이상(로비/전투/보스), SFX 10종 이상, 음량 설정 저장 | 수동 청음 + 설정 저장 테스트 | ❌ 미충족 — SND-01/02 미착수, `public/assets/audio` 오디오 에셋 0개 |
| DoD-12 | **품질 게이트**: 번들 ≤ 700KB gzip, 초기 로딩 ≤ 3초(4G), 전투 60fps, iOS Safari/Android Chrome 동작 | `build:analyze` + 실기기 | 🔶 부분 — 번들 568KB gzip으로 임계값 충족. 실기기(iOS Safari/Android Chrome) 및 로딩 시간·60fps 측정(QA-03)은 미실행 |
| DoD-13 | **운영 준비**: 에러 모니터링 연동, 이벤트 기간 자동 스케줄(하드코딩 종료일 제거), 확률 고지 UI, 세이브 export/import | 운영 체크리스트 | 🔶 부분 — 이벤트 기간 자동화(FIX-04)와 확률 고지 UI(`RateDisclosurePanel`, BLK-07)는 완료. 에러 모니터링(OPS-01)·세이브 export/import(OPS-02 잔여)는 미착수 |
| DoD-14 | **문서 SSOT**: CURRENT_STATUS와 실제 코드 일치, PRD v5.2의 구버전 "치명적 이슈" 목록 정리(해소/잔존 표기) | 문서 감사 | 🔶 부분 — CURRENT_STATUS.md·본 문서는 이번 갱신으로 2026-09-03 실측과 일치. `PRD_Unified_v5.md` 자체의 이슈 목록 해소/잔존 마킹 작업은 별도 미완 |
...
| **BAL-02** | 전투 밸런스 곡선: 스테이지 25개 + 탑 100층 DPS 요구치 vs 파티 성장곡선 대조 | M | DATA-01, MECH-02 | 챕터별 권장 전투력 표 산출, 이탈 구간 없음 | 🔶 부분 완료 09-02~03 (`docs/balance/BALANCE_DESIGN_v1.md` v1.2) — **스테이지 12/25가 턴수 밴드 이탈, §9-4 조정안 미적용** |
...
| ID | 태스크 | 난이도 | 의존성 | 수용 기준 | 상태 |
|----|--------|--------|--------|----------|------|
| **QA-01** | 전면 회귀: vitest(≥685)+tsc+build+Playwright 34 전량 실행, 결과 CURRENT_STATUS 반영 | S | TRACK A 완료 | DoD-1, 2 충족 | ✅ 09-03 (58파일 1476 통과/tsc 0/build 성공/e2e 전량 통과, CURRENT_STATUS 반영) |
| **QA-02** | 코어 루프 수동 E2E 5종: 가챠→전직→전투→저장 / 탑 50층 / PvP / 길드 / 이벤트 던전 | M | TRACK A, TOWER-02 | 시나리오별 체크리스트 통과 (DoD-3, 4) | 🔶 부분 — 자동 e2e(`battle-cult-live`/`ascension-cutscene`/`onboarding-full`)가 가챠→전직→전투 루프를 커버. **탑 50층/PvP/길드 수동 체크리스트는 미실행** |
| **QA-03** | 모바일/크로스브라우저: iOS Safari, Android Chrome, 18:9~20:9 비율, 번들≤700KB·로딩≤3초·60fps 측정 | M | QA-01 | DoD-12 충족 리포트 | ❌ 미착수 |
| **OPS-01** | 에러 모니터링(Sentry 또는 경량 대안) 연동 + sourcemap 업로드 | S | FIX-05 | 프로덕션 에러가 대시보드에 수집됨 | ❌ 미착수 |
| **OPS-02** | 세이브 export/import UI + 확률 고지 화면(가챠 정보 팝업에 RATES 표기 — 이미 부분 존재, 법적 문구 보강) | S | QA-01 | export→import 왕복 무손실 | 🔶 부분 — 확률 고지 UI(`RateDisclosurePanel`, T-13)는 09-02 웨이브 2에서 구현·BLK-07 해소. **세이브 export/import는 여전히 미착수** |
| **DOC-01** | 문서 SSOT 정리: PRD v5.2 이슈 목록에 해소/잔존 마킹, 본 WBS를 CURRENT_STATUS 백로그로 흡수 | S | FIX-06 | DoD-14 | ✅ 09-03 (이 갱신 — WBS 상태·부록 C·DoD 표 전부 실측 반영) |
...
## 부록 C — 스토리 모드 기획 중 발견된 릴리스 차단 결함 (2026-09-02 추가)

`docs/STORY_MODE_GDD.md` §7-1에서 game-design-director가 데이터 대조로 확인한 기존 저장소 결함. 스토리 모드가 만든 문제가 아니라 **이미 존재하던 결함**이며, 갭 테이블(§2-2)의 P0로 취급한다.

> **2026-09-03 기준**: 7건 전부 웨이브 1 커밋 `1a9c27e`에서 해소 확인. `npm run test:e2e:story`(114/114) 및 vitest 1476/1476 통과가 회귀 없음을 뒷받침한다.

| # | 이슈 | 영향 | 매핑 갭 | 우선순위 | 상태 |
|---|------|------|--------|---------|------|
| BLK-01 | 스테이지 참조 적 38종 중 30종이 `enemies.json` 미정의(보스 4종 포함). 8종은 리네임으로 해소 | 챕터 2~5 플레이 불가 | G-10/G-05 | **P0** | ✅ 해소 (`1a9c27e`) |
| BLK-02 | 가챠 N등급 풀 공백 → 10연 중 평균 6개 `null` | 첫 무료 10연 불성립 | G-10 | **P0** | ✅ 해소 (`1a9c27e`) |
| BLK-03 | `calculatePower` 2개 병존(`data/index.ts`, `ProgressionSystem`), 양쪽 모두 장비 미반영. 4인 상한 ≈5,484 < 4-3 요구 5,600 | 챕터 4-3 이후 13스테이지 도달 불가 | BAL-02 | **P0** | ✅ 해소 (`1a9c27e`) — 단, BAL-02 자체(스테이지 12/25 턴수 밴드)는 잔존, §3 참고 |
| BLK-04 | 레이어2 재화(기관 각인서/에센스/각성의 불꽃) 획득처 0 | 전직 불가 | G-08 | **P0** | ✅ 해소 (`1a9c27e`) |
| BLK-05 | 소환 에너지 10/pull → 10연이 일일 탱크 98% 소모 | 첫 10연 후 진행 정지 | BAL-01 | P0 | ✅ 해소 (`1a9c27e`) |
| BLK-06 | `daily_004`/`daily_005` 도달 불가 → 일일 젬 110 중 80 차단 | Day2~3 경제 붕괴 | BAL-01 | P0 | ✅ 해소 (`1a9c27e`) |
| BLK-07 | 소환 화면 확률 고지 UI 부재 | 법적 요구, 가챠 재활성화 차단 | OPS-02 | **P0** | ✅ 해소 (`RateDisclosurePanel`, T-13, 09-02 웨이브 2 `0096ec6`) |

구현 순서는 `STORY_MODE_GDD.md` §9 웨이브 1(T-D1/T-D2/T-S1/T-S2)을 따랐다.

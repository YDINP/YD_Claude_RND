...
| # | 이슈 | 영향 | 매핑 갭 | 우선순위 |
|---|------|------|--------|---------|
| BLK-01 | 스테이지 참조 적 38종 중 30종이 `enemies.json` 미정의(보스 4종 포함). 8종은 리네임으로 해소 | 챕터 2~5 플레이 불가 | G-10/G-05 | **P0** |
| BLK-02 | 가챠 N등급 풀 공백 → 10연 중 평균 6개 `null` | 첫 무료 10연 불성립 | G-10 | **P0** |
| BLK-03 | `calculatePower` 2개 병존(`data/index.ts`, `ProgressionSystem`), 양쪽 모두 장비 미반영. 4인 상한 ≈5,484 < 4-3 요구 5,600 | 챕터 4-3 이후 13스테이지 도달 불가 | BAL-02 | **P0** |
| BLK-04 | 레이어2 재화(기관 각인서/에센스/각성의 불꽃) 획득처 0 | 전직 불가 | G-08 | **P0** |
| BLK-05 | 소환 에너지 10/pull → 10연이 일일 탱크 98% 소모 | 첫 10연 후 진행 정지 | BAL-01 | P0 |
| BLK-06 | `daily_004`/`daily_005` 도달 불가 → 일일 젬 110 중 80 차단 | Day2~3 경제 붕괴 | BAL-01 | P0 |
| BLK-07 | 소환 화면 확률 고지 UI 부재 | 법적 요구, 가챠 재활성화 차단 | OPS-02 | **P0** |

구현 순서는 `STORY_MODE_GDD.md` §9 웨이브 1(T-D1/T-D2/T-S1/T-S2)을 따른다.

...
작성일: 2026-09-02
상태: Phase 0 골격 구현 완료 (2026-09-02). 실제 텔레그램 클라이언트 검증은 봇 토큰·공개 URL 확보 후
...
└── tools/
    ...
    └── theme-gen/      # ComfyUI 심볼 세트 생성 파이프라인 (기존 로컬 세팅 재활용)
```
...
| 1 엔진 | slot-engine + rtp-sim CLI + 결정론 테스트 | classic-777 math.json이 시뮬에서 RTP 96% ±0.5 | 1주 |
| 2 첫 게임 | renderer + classic-777 + 서버 spin API + 원장 | 실제 서버 스핀이 end-to-end로 돌고 잔액 불변식 통과 | 2주 |

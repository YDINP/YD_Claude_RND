...
작성일: 2026-09-02
상태: Phase 5 진행 중 (2026-09-02). Phase 0~3·아트·UX 1·2차·경제 재조정(총 RTP 96%)·검수 CLI/GUI 완료. 2호 Fruit Fiesta 구현·아트 완료, 검수 모듈 5릴 지원 마무리 중. 실제 텔레그램 클라이언트 검증은 봇 토큰·공개 URL 확보 후
...
| 5b 웨이브 1 (완료, 2026-09-03) | royal-diamond-777(미스터리, 해석적 94.47%) · sheriff-sixgun(확장 와일드+갬블, MC 94.36%) · shiba-shrine(243웨이즈+랜덤 와일드, 와일드 접기 재튜닝 후 MC 94.39%). 엔진 변이 파이프라인·웨이즈·MC 디스패처·갬블 스키마, API 변이 전달·갬블(에스크로·멱등키), 렌더러 변이 연출·웨이즈 표시, hub 갬블 UI·웨이즈 도움말, 아트 3게임 42장(Codex 재시도 루프) + 5릴 정사각·가로형 창 프레임 v3. 파트별 검수 각 1회 반려→수정→엔진 재검수 APPROVED, 정식 감사 5/5, 3게임 브라우저 e2e | 3게임 로비 노출·e2e·검수 리포트 | 완료 |
| 5 양산 (진행 중) | 2호 Fruit Fiesta(5×3·20라인·스캐터·프리스핀 ×2·10회·재발동). 엔진: 스캐터/프리스핀/5릴 해석적 RTP(닫힌식, 500만 스핀 MC와 0.07SE 일치). API: game_states 프리스핀 세션(무차감·고정 베팅·잭팟/xp 제외). 렌더러: 스캐터 링·버스트·프리스핀 모드. hub: 프리스핀 자동 진행·배너·상태 복구. 아트 13장 Codex 생성 | 기본 RTP 94.493% + 잭팟 1.5% = 95.99%, 적중률 30.7%. **소요: 2026-09-02 하루(병렬 5팀)** | 2026-09-02 |
...
| 2026-09-03 | 당첨 표시는 선 대신 이펙트 | 페이라인 폴리라인 제거, 빛 펄스가 당첨 심볼 경로를 훑으며 도착 시 fx 발화(`paylineStyle:'effect'`), 등급별 훑기 속도 |
| 2026-09-03 | 스핀 중 터치/스페이스로 즉시 결과 | `SpinHandle.skip()` 260ms 착지, hub 스테이지 탭·Space(대기 중 Space=스핀, 반복 무시) |
| 2026-09-03 | 릴이 화면을 꽉 채우도록 프레임 세로 위주 | 프레임 v2 재생성(얇은 레일, 창 82~84%×63%), 창 폭 = 컨테이너 폭, 심볼 125px |
| 2026-09-03 | 프리스핀 전환 연출 + 전용 배경 | 섬광→방사형 와이프 700ms, `backgroundFreeSpins`(theme-gen `bgFreeSpins`), `modeTransition` 이벤트로 배너·자동스핀 동기화 |
| 2026-09-03 | 베팅 리스트 선택, 페이지형 도움말, 당첨 금액은 하단 | 베팅 시트, 도움말 탭(배당표/페이라인/피처/공정성), WinStrip(컨트롤 위 44px), 등급 배너는 단어만 |
| 2026-09-03 | 당첨 연출 스프라이트 아틀라스 | theme-gen `sheet` kind(컨택트 시트 1장→9프레임 아틀라스), 렌더러 AnimatedSprite 재생, classic-777 seven/wild 프로토타입 |
| 2026-09-03 | UX 4차: 당첨 연출 순환 · 릴 밖 텍스트 · 디버그 패널 · 스핀 속도 | 이동 흰빛 제거(정지 테두리+fx), A→B→A 순환(라인당 1.9s), WinStrip 중앙정렬+순환마다 심볼별 문구, 릴 위 명판·프리스핀 카운터 제거, 개발자 디버그 패널(프리셋 5종·연출 재생·타이밍·응답 JSON), 스핀속도 보통/빠름/터보, 스핀→스탑 버튼, 상단 WIN 배너 제거, 프레임 좌우 잘림 제거(overflowX 0)+창 확대 v4, 로비 카드 3:2 |
| 2026-09-03 | 신규 슬롯 10종 기획·개발 오케스트레이션 | `docs/GAME_CATALOG.md`(다양한 컨셉·신규 메커닉·3웨이브), 웨이브 1 착수(royal-diamond-777·sheriff-sixgun·shiba-shrine, 엔진 변이/웨이즈/MC RTP, 아트 41장) |
| 2026-09-02 | 설정이 없음(언어 등) | 헤더 톱니 → 설정 시트(언어 자동/EN/KO, 사운드, 햅틱, 모션 줄이기, 공정성 설명, 버전/지원 ID). 언어는 `PATCH /me`로 서버 저장, 로그인이 덮어쓰지 않음 |

## 12. 기능 추가 표준 절차 (2026-09-03 확정, 매 기능 필수)

| 단계 | 내용 | 담당 |
|---|---|---|
| 1 구현 | 파트별 실행자(엔진/API/렌더러/hub/아트) 병렬 구현. 파일 소유 분리 | executor |
| 2 파트별 검수 | 파트마다 읽기 전용 리뷰어가 정확성·보안·계약 준수·테스트 커버리지 확인. blocker/should-fix/nit 등급 | architect / code-reviewer |
| 3 통합 검증 | typecheck·전체 테스트·빌드 + 브라우저 e2e(로비·게임·해당 기능 화면) + 필요 시 RTP 검수(`run audit`) | 팀 리드 |
| 4 고도화 점검 | "더 좋게 만들 수 있는 것" 목록화: 성능·접근성·연출 품질·경제 밸런스·운영 편의. 즉시 반영/백로그 분류 | architect + 팀 리드 |
| 5 기록·커밋 | `docs/QA_LOG.md`에 기능·검수 결과·고도화 항목 기록, 계획서 갱신, 커밋·푸시 | 팀 리드 |

should-fix는 커밋 전 반드시 반영, nit는 백로그. 고도화 항목 중 즉시 반영 가능한 것은 같은 라운드에서 처리한다.

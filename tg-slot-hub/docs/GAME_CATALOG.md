...
| RTP 산출 | **monte-carlo** 20M, SE ≈ 0.04%p — 드롭의 배치 배타성이 릴 간 상관을 만든다 |
| 공수 | 12 / 6 / 2 / 11 h |
...
| RTP 산출 | **monte-carlo** 20M, SE ≈ 0.04%p — 연쇄가 그리드 전체 상태에 의존해 닫힌 식이 없다 |
| 공수 | 20 / 14 / 3 / 11 h |
...
| RTP 산출 | **monte-carlo** 20M, SE ≈ 0.05%p — 오브 100× 꼬리 때문에 SE가 다른 게임보다 크다 |
| 공수 | 14 / 12 / 3 / 12 h |
...
| RTP 산출 | **monte-carlo** 20M, SE ≈ 0.05%p — 양방향 자체는 해석 가능하지만 미터가 스핀 간 상태를 만들어 정상분포가 필요하다 |
| 공수 | 12 / 10 / 6 / 13 h |
...
| P3 cascade | **`monte-carlo`** SE ≤ 0.05%p (20M) | 연쇄 길이와 배수가 그리드 전체 상태에 의존, 상태공간이 심볼^30 |
| P4 cluster / pay-anywhere | **`monte-carlo`** SE ≤ 0.05%p (20M) | flood-fill 그룹 크기 분포가 강한 공간 상관을 갖는다 |

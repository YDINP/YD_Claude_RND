...
현재 저장소에 있는 유일한 전환물인 `sheriff-sixgun`의 모래폭풍(`fs-enter.webm`)은
**서부라는 배경만 맞고 기계는 없다** — 이 문서는 그것을 교체 대상으로 본다.
...
**둘 다 기능상 필요한 자리**다 — 복귀 머리가 420ms 스왑 시점을 덮지 못하면 교체가 노출된다.
...
`opaqueMs`를 0이 아닌 값으로 바꾼다면 **500ms를 넘기지 말 것** — sheriff와 shiba부터 교체가 노출된다.
...
| 종료 | 3.00초 뒤 오버레이 제거. 마지막 프레임에서 정지하므로 남겨 두면 화면을 계속 가린다 |
...
v3 재발주는 승인됐으나 계정 한도로 **거부되어 과금되지 않았다**(3-2 참조).

**막힌 것 / 대기 중**

| 항목 | 비용 | 상태 |
|---|---:|---|
| `sheriff-sixgun` 진입 v3 | $0.93 | **차단.** `doubao-seedance-2-0` 계정 추론 한도(`SetLimitExceeded`). 모델 활성화 페이지에서 Safe Experience Mode / 지출 한도 조정 필요 |
...
## 8. 재발주 대기 프롬프트 (v4 — R12 적용, **승인 전 발주 금지**)

기존 8편은 **팩의 `art/prompts.json` 지침을 정면으로 위반한 실사 영상**이다(R12 참조).
4팩을 같은 기준으로 다시 뽑는다 — fruit만 고치면 fruit만 스타일라이즈드이고 나머지는 실사인,
지금보다 나쁜 «섞인» 세트가 된다.

- **R10대로 팩당 한 편**(한 클립에서 진입·복귀를 잘라냄) → 4 x $0.93 = **$3.72**
- shiba 단색 정지 재작업과 sheriff 진입 v3 재발주가 **여기 흡수**된다. 따로 쓰지 않는다.
- 기존 8편은 **지우지 않는다.** 새 것이 더 나쁘면 돌아갈 자리다.
...
**받은 뒤 판정 순서** — 한 편이라도 걸리면 그 편만 기존 것을 유지하고 나머지는 교체한다.
전부 아니면 전무가 아니다.

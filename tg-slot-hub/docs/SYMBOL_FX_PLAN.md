...
## 8. UX 라운드 4 (2026-09-03) — 당첨 연출 순환과 릴 밖 텍스트

사용자 피드백으로 확정된 규칙이다.

- **이동 광원 삭제.** 당첨 경로를 훑고 지나가던 흰 빛(`pulse.ts`·`pixi/winPulse.ts`)을 제거했다. 당첨 강조는
  **정지 테두리 + 심볼 fx**만 쓴다. phase A(전체 동시 표시)와 phase B(라인별)가 같은 `drawWinGlow`를 쓴다.
- **순환 재생.** 승리 연출은 A→B→A로 다음 스핀까지 계속 돈다. `showWins()`는 첫 바퀴에서 resolve하고
  루프는 배경에서 이어진다. 타이밍은 phase A 1260ms(등급별 ×1.78/×2.44), phase B 라인당 1900ms, 크로스페이드 220ms.
- **릴 위 텍스트 금지.** 캔버스에 그리던 라인 명판과 프리스핀 카운터를 모두 걷어내고, 그 정보는 릴 아래
  WinStrip에서 보여준다. 캔버스에는 심볼과 연출만 남는다.
- **WinStrip 표기.** 중앙 정렬 세로 배치: [작은 라벨] / [총액] / [라인 문구(고정 높이)] / [받기·더블(있을 때)].
  라인 문구는 순환마다 갱신된다 — 라인 `세븐 ×3 · 100`, ways `타이거 ×5 · 4 ways · 348`, 그룹 `바 아무거나 ×3 · 50`,
  순환 총액 `합계 348`. 더블업 제안이 떠 있어도 문구는 계속 표기되고 버튼은 자리를 지킨다.
- **스킵 규칙.** 연출 중 화면 탭은 아무것도 하지 않는다(연출을 끊지 않는다). 릴 회전만 스탑 버튼·탭·스페이스로
  건너뛴다. 스핀 버튼은 연출 중에도 활성이며 누르면 다음 스핀으로 넘어간다.
- **스핀 속도.** `setSpinSpeed('normal'|'quick'|'turbo')` — 5릴 기준 1.65s/0.9s/0.45s. phase B는 읽는 시간이라
  속도와 무관하게 1900ms를 지킨다. 모션 축소 설정이 항상 우선한다.

### 스프라이트 시트 실측과 메모리 백로그 (2026-09-03)

12장 생성 완료(shiba/sheriff/royal/fruit 각 3종: 대표심볼·wild·scatter, 3×3 9프레임 12fps).
`theme.json.sheets[symbol].win`이 사이드카 json을 가리키고 렌더러가 AnimatedSprite로 재생한다.

⚠ **해상도가 과하다.** `outSize: 768` → 프레임 256px인데 실제 화면 심볼은 약 86px(430px 기기)이다.
디스크 4.8MB, GPU에서는 압축이 풀려 768×768×4 ≈ 2.25MB/장 × 게임당 3장 ≈ 6.75MB가 상주한다.
- 1차 대응: `outSize`를 576(프레임 192px)으로 낮추고 `--reprocess`로 raw에서 재처리 → 디스크·GPU 약 44%
- 2차 대응: 승리 연출이 끝나면 시트 텍스처를 해제하고 다음 당첨에서 다시 올리는 수명 관리
- 측정 먼저: 게임 진입 후 `renderer.info`/텍스처 카운트로 실제 상주량을 재고 판단한다

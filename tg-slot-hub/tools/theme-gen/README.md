...
## 전환 클립 검사 (`check:clips`)

```bash
pnpm --filter @tgslot/theme-gen check:clips              # games 전체
pnpm --filter @tgslot/theme-gen check:clips <파일|폴더>   # 특정 대상
```

`games/<pack>/theme/transitions/*.webp`(프리스핀 진입·복귀 커튼)가 재생 계약을 지키는지 본다.
계약 원본은 `docs/TRANSITION_DESIGN.md` 5-1이고, 여기서는 **파일만 보고 알 수 있는 것**을 검사한다.

| 항목 | 기준 |
|---|---|
| 해상도 | 360x640 |
| 길이 | 3.00초 ±40ms (16fps 한 프레임 62.5ms보다 작은 오차 = 프레임 하나 빠진 클립을 잡는다) |
| 루프 | `loop=1` — 한 번 재생하고 멈춘다. `0`은 무한 반복이라 커튼이 안 걷힌다 |
| **정지 총량** | 클립 길이의 **20% 이하** |

### 정지 총량이 왜 판정 기준인가

libwebp는 **동일한 연속 프레임을 한 장으로 합치고 지속시간만 늘린다.** 그래서 유난히 긴
프레임은 곧 «그 동안 화면이 멈춰 있었다»는 뜻이다. 전면을 덮는 커튼이 멈추면 연출이 아니라
로딩 화면으로 읽힌다.

다만 **프레임 하나가 길다는 것만으로 실패시키면 안 된다.** 전환에는 기능상 반드시 정지해야
하는 자리가 있다 — 복귀 클립의 머리는 화면 교체 시점을 가려야 하고, 진입 클립의 꼬리는
교체가 끝난 뒤 커튼이 버티는 구간이다. 둘 다 단색이어도 정상이다.

그래서 200ms 넘는 프레임을 «정지 한 칸»으로 **세기만** 하고, 판정은 그 **합계 비율**로 한다.
실측값이 기준을 정했다:

| 클립 | 정지 총량 비율 | 판정 |
|---|---:|---|
| sheriff / fruit | 0% | 통과 |
| royal | 0% (최대 프레임 187ms — 200ms 미만이라 세지 않는다) | 통과 |
| shiba (재컷 후) | 10.4% / 12.5% | 통과 |
| **shiba (재컷 전)** | **35.0% / 36.7%** | **실패** — 3초 중 1.3초가 단색 정지였다 |

20%는 통과시켜야 할 최댓값(12.5%)과 잡아내야 할 최솟값(35.0%) 사이에 넉넉히 들어간다.
값은 `constants.ts`의 `TRANSITION_CLIP_HOLD_FRAME_MS` / `TRANSITION_CLIP_MAX_HOLD_RATIO`다.

> 이 검사가 없던 동안 shiba의 단색 1.3초는 `theme.json` 배선과 `pack:check` 통과까지
> 그대로 지나갔다. 재생은 멀쩡했기 때문이다. 눈으로 보기 전에는 아무도 몰랐다.
...
- 전환 클립 검사(`transitionClipCheck`): RIFF 청크 파싱(캔버스 크기·루프·프레임 지속시간),
  모르는 청크 건너뛰기, RIFF/WEBP 아닌 파일·ANMF 없는 정지 WebP·청크 길이가 파일을 벗어난
  잘린 파일 거부, 해상도/길이/루프 위반 검출, 그리고 정지 총량 판정 —
  재컷 전 shiba(35%)는 실패하고 재컷 후 shiba(12.5%)와 royal(187ms 한 칸)은 통과하는지

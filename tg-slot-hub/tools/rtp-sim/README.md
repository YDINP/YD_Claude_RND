...
뮤테이션 중 `randomWild`처럼 배치가 배타적인 것, 스핀 간 이월이 있는 것, 캐스케이드·클러스터·
리스핀은 릴 독립이 깨져 닫힌 식이 없다. 이때는 RTP 자체가 표본 추정이 되므로 게이트가 하나 더 붙는다.

| 조건 | 요구 |
|---|---|
| 목표와의 거리 | `\|rtp − rtpTarget\| ≤ 0.5%p` |
| 정밀도 | 95% 신뢰구간 반폭 ≤ 0.2%p (넘으면 **표본 부족**으로 FAIL) |
| 기록 | `spins`·`seed`·`stderr`·`ci95`를 리포트에 필수로 남긴다 |
| 베팅 레벨 | 레벨마다 따로 50만 스핀을 돌려 각각 ±0.5%p |

베팅 레벨을 레벨마다 다시 돌리는 이유는, RTP가 베팅액과 무관하다는 보장이 이 모델에는 없기 때문이다.
지급 반올림이나 상한이 레벨에 따라 다르게 물릴 수 있다.

> 카탈로그 초안의 `≤ max(0.5%p, 3×SE)`는 쓰지 않는다. 그 식은 표본이 부실할수록 허용 오차가
> 넓어져 느슨한 측정이 오히려 통과하기 쉬워진다. 거리와 정밀도를 분리한 위 조건이 확정 규칙이다.

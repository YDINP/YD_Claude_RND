...
화면의 모든 숫자는 어디서 나왔는지 배지를 단다.

| 배지 | 뜻 |
|---|---|
| **전수조사** | 모든 정지 조합을 계산. 표본 오차 없음 |
| **표본 추정** | RTP는 닫힌 식의 정확값, 분포·기여도만 표본 |
| **몬테카를로** | RTP까지 표본. 신뢰구간이 함께 뜨고 게이트가 그 폭을 본다 |

몬테카를로 모델은 "RTP 정밀도" 표에 스핀 수·시드·표준오차·95% 신뢰구간을 남긴다.
반폭이 0.2%p를 넘으면 게이트가 표본 부족으로 떨어뜨린다.

`payModel: 'ways'` 게임은 페이라인이 없으므로 라인 표가 **웨이즈 수 분포**로 바뀐다.
뮤테이션이 있는 게임은 종류별 발동 빈도 표가 붙고, 샘플 스핀이 변형 전후 격자를 나란히 보여 준다.

 ...
 */
import type { McDone, McRequest, McResponse } from './mcTypes.js'
...
/**
 * 몬테카를로를 워커에서 돌린다. 진행률은 콜백으로, 결과는 프로미스로 온다.
 *
...
export function runMcInWorker(
  ...
  const promise = new Promise<McDone>((resolve, reject) => {
    abort = reject
    worker.onmessage = (event: MessageEvent<McResponse>) => {
      ...
      settled = true
      worker.terminate()
      resolve({ mc: message.mc, ruin: message.ruin })
    }

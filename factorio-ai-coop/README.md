...
헤드리스 서버는 **정상 종료할 때만** 원본 세이브에 되쓴다. 창을 닫거나 프로세스를 죽이면
아무것도 저장되지 않고, 서버 설정의 `autosave_interval`이 만드는 `_autosave1.zip`은
다음 시작 때 **불러오지 않는 별도 파일**이라 소용이 없다.

그래서 월드가 **5분마다 자기가 실행 중인 세이브 파일에 스스로 되쓴다.** 게임 채팅에서
`저장`이라고 하면 즉시 저장하고, 에이전트 데몬도 종료할 때 한 번 저장한다.
최악의 경우에도 5분치만 잃는다.

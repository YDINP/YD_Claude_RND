...
광맥에 채굴기와 상자를 놓고 연료를 채운다. 그다음은 비축분을 유지한다.

**같은 일을 둘이 하지 않는다.** 계획 함수는 "다음에 할 일" 하나가 아니라 **지금 할 만한
일의 목록**을 우선순위대로 내놓고, 크루가 작업을 하나씩 선점 배분한다. 작업마다 키가 있고
(`furnace`, `gather:coal`, `smelt:3,3`, `automate:iron-ore`, `stock:copper-ore`) 한 키는
한 명만 쥔다. 그래서 실제로는 이렇게 갈린다:

```
[alpha]   돌부터 캐서 화로를 만들겠습니다.      ← furnace
[bravo]   연료가 없습니다. 석탄 캐러 갑니다.     ← gather:coal
[charlie] 철광석 캐러 갑니다.                  ← gather:iron-ore
```

화로는 force가 공유하므로 한 명만 지으면 전원이 쓴다. 다만 제련 작업은 **화로 좌표로**
키를 만들어 한 화로에 한 명만 붙는다 — 둘이 같은 화로에 광석을 밀어넣고 둘 다 산출을
기다리는 건 협업이 아니다.

**재료는 게임에게 물어본다.** 채굴기 레시피에는 화로가 들어가서 철판만으로는 못 만든다.
예전엔 그걸 모르고 "자동화 준비 → 제작 실패"를 무한 반복했다. 지금은 `get_craftable_count`로
만들 수 있는지 먼저 확인하고, 안 되면 모자란 재료를 캐러 간다.

**실패한 판단은 2분간 봉인한다.** 닿지 않는 화로 하나가 같은 대사로 채팅을 채우는 일을
막는다. 그동안 에이전트는 목록의 다음 일로 내려간다.

`--manual`을 주면 지시만 기다린다. 게임 안에서 `수동`/`알아서 해`로 개별 전환도 된다.

## 화면에서 알아보기

AI 캐릭터는 플레이어가 아니라서 게임이 이름표도, 미니맵 점도 그려주지 않는다. 둘 다 직접
그리며, 서로 다른 메커니즘이다.

- **머리 위 이름표** — 렌더링 객체가 캐릭터를 따라다닌다. 이름과 지금 하는 일이 같이 뜬다
  (`alpha · mine`). 에이전트마다 색이 다르다.
- **미니맵/지도 표시** — 맵 태그. 월드에 그린 렌더링 객체는 지도에 안 나오므로 이것만이
  방법이다. 맵 태그는 위치를 바꿀 수 없어 지우고 다시 만들어야 하는데, 6타일 이상 움직였을
  때만 갱신해서 초당 두 번씩 태그를 갈아치우지 않는다.

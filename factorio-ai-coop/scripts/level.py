...
** 21회차에 사용자 지시로 세웠다. 지금은 쓰지 않는다. **

    사용자: "상자용량제한은 왜걸은겆지?"
    사용자: "아니상자가 부족하면 더 늘리고 물류구조를 계속 개선해나가면 되잔아"

제한에 닿은 상자의 팔은 서 버린다. 그래서 «받는 팔 수»가 줄고, 선반이
거의 찼을 때는 덜 찬 두세 상자만 일한다 - 판금 간선이 막히는 데 한몫했다.
석탄이 바닥인 날에도 석탄 선반에 걸려 있었다. 고르게 차는 것은 보기에만
좋았고, 모자란 것을 고르게 퍼뜨렸을 뿐이다.

한쪽부터 차는 것은 이 구조의 본래 모습이고, 문제가 되는 때는 «다 찼을
때»뿐이다. 그 답은 racks.py 다 - 차기 전에 상자를 늘린다.

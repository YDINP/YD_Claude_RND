...
# 안쪽 간격.
#
# 34 로 두었다. 「테두리가 뚫린 날 시간을 벌 만큼만」이라는 뜻이었는데,
# 사거리가 18 이라 34 간격은 사이에 구멍을 남긴다. 실측(21회차):
#
#     덮인 건물 86/111 = 77% 에서 멎음
#     남은 16채가 전부 철광 밭 한가운데 (x -82..-73, y 48..52)
#     가장 가까운 포탑까지 20~24칸
#
# 둘레는 섰는데 안쪽이 비었다. 그런데 20회차에 죽은 여섯은 «밭에서 일하던
# 사람들»이다 - 기지는 멀쩡했고 밖에 나간 사람만 죽었다. 밭 안쪽은
# 「시간을 버는 곳」이 아니라 «사람이 서 있는 곳»이다.
#
# 대각선까지 이으려면 간격이 18*sqrt(2) 보다 좁아야 한다. 24 면 이어진다.
INNER_GAP = 24
...
def plan_seats(ai):
    ...
    wanted = []
    # «아직 안 덮인 것» 옆을 «맨 먼저» 짚는다.
    #
    # 여기까지의 자리표는 전부 «격자»에서 나온다 - 테두리는 상자를 따라,
    # 안쪽은 일정 간격으로. 격자는 「어디가 비었나」를 모른다. 실측(21회차):
    #
    #     덮인 건물 86/111 = 77% 에서 여러 순번째 멎음
    #     남은 16채는 철광 밭 한가운데, 가장 가까운 포탑까지 20~24칸
    #     그 둘레 16칸 안에 포탑을 놓을 수 있는 칸이 «116곳»
    #
    # 땅이 없어서가 아니라 «짚지 않아서» 안 덮였다. 격자를 촘촘히 하면
    # 언젠가 걸리지만, 그것은 우연을 기다리는 것이고 그동안 엉뚱한 빈 땅에
    # 포탑이 선다.
    #
    #     안 덮인 것을 덮으려면 «안 덮인 것»을 물어야 한다.
    #
    # unguarded() 가 이미 그 답을 들고 있었다. 먼 것부터, 그 곁을 짚는다.
    #
    # 그리고 이 자리들이 목록 «맨 앞»에 와야 한다. 처음에는 뒤에 붙였는데
    # 아무 소용이 없었다 - 자리표에서 앞에서부터 세 개를 뽑으므로, 앞의
    # 격자 자리가 남아 있는 한 구멍 자리는 영영 차례가 안 온다.

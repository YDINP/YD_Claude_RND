"""손으로 돌릴 수 있는 숫자들. 왜 그 값인지는 각 줄의 주석에 있다.

한곳에 모아둔 이유는 튜닝할 때 파일을 뒤지지 않기 위해서다. 여기 없는
숫자가 코드 안에 박혀 있다면, 그것은 튜닝값이 아니라 게임의 사실이다.
"""

from __future__ import annotations


# Call signs are ASCII so they survive Lua, JSON and chat without surprises.
# "1번" addressing exists for anyone who would rather not type them.
CALL_SIGNS = ["alpha", "bravo", "charlie", "delta", "echo", "foxtrot", "golf", "hotel"]


DRILL = "burner-mining-drill"


CHEST = "iron-chest"


DRILL_FUEL = 10


FURNACE_FUEL = 5


SMELT_BATCH = 20


PLATES_FOR_TOOLS = 12   # enough to hand-craft a drill and a chest


STOCKPILE = 30


BACKOFF_SECONDS = 120


# 길이 없어 실패한 일감은 그 사람에게 오래 막아둔다. 호수는 다음 배차 때도
# 그 자리에 있다.
UNREACHABLE_QUIET = 900.0


# 같은 자리에서 길찾기가 이만큼 연달아 실패하면 갇힌 것으로 본다. 한 번은
# 운이 나쁜 것이고, 세 번은 지형이다.
STUCK_STRIKES = 3


# 창고에 이만큼 쌓였으면 그만 캐도 된다. 485분째에 돌이 48,099개 있는데
# 채굴기 22대가 돌로 꽉 찬 상자 앞에서 여전히 서 있었다. 캐는 것도 일이고
# 막힌 채로 서 있는 것도 자리다.
SURPLUS = 6000


# 채굴기 밑 2x2 에 이만큼도 안 남았으면 걷어내서 두꺼운 자리에 다시 세운다.
# 400이면 0.25/s 로 27분이고, 옮기는 데 드는 것은 한 번의 걸음이다. 같은
# 광맥의 두꺼운 칸은 보통 2,000이 넘는다 - 다섯 배 넘게 남는 장사다.
#
# 완전히 마를 때까지 기다리는 것은, 그 자리에서 몇 분 더 캐려고 이십 분을
# 버리는 일이다.
THIN_DRILL = 400

# 세우는 문턱. 걷어내는 문턱(THIN_DRILL)보다 «반드시» 높아야 한다.
#
# 14회차에 둘이 같았던 적은 없지만 더 나빴다 - 자리표는 「광석에 닿기만
# 하면」 세우라 했고 점검은 「400 미만이면」 걷으라 했다. 열 자리 중
# 일곱이 문턱 밑이었고 두 자리는 광석 26개, 즉 104초짜리였다. 세우면
# 걷고 걷으면 세우는 동안 그 줄에 깔던 벨트까지 같이 뜯겨서, 세 시간
# 동안 도는 채굴기가 넷을 넘지 못했다.
#
# 여유는 시간으로 정한다. 세운 자리는 화로 한 판이 도는 동안(30분)은
# 걷어내는 선 위에 있어야 한다. 버너 채굴기는 초당 0.25개를 캔다.
DRILL_PER_SECOND = 0.25
DRILL_SETTLE_MIN = 30
RICH_DRILL = THIN_DRILL + int(DRILL_PER_SECOND * 60 * DRILL_SETTLE_MIN)  # 850


# 벨트 한 줄로 이을 수 있는 최대 거리. 이보다 멀면 벨트값이 손으로 나르는
# 값을 넘는다 - 노란 벨트 하나가 철판 1 + 기어 1(= 철판 3)에 두 칸이다.
BELT_REACH = 40


# 한 줄을 놓기 전에 손에 쥐고 있어야 할 여유분. 중간에 모자라 끊긴 벨트는
# 안 놓은 것과 같고, 이미 쓴 자재만 사라진다.
BELT_SPARE = 4   # how long a failed kind of work stays off the ladder


# Each agent takes one resource so a crew does not all stand on the same patch.
FOCUS_ORDER = ["iron-ore", "coal", "copper-ore", "stone"]


# 2.0 opens its first technologies with an action rather than science packs:
# ten copper plates smelted opens electronics (circuits, lab, inserters), fifty
# iron plates opens steam power (pipes, boiler, engine, pump). Nothing else can
# start until those two are in, because the lab itself is behind electronics.
SMELT_FOR_TECH = [
    ("electronics", "copper-ore", "copper-plate"),
    ("steam-power", "iron-ore", "iron-plate"),
]


ORE_BATCH = 25


# 상자가 이 거리 안에 있으면 그 채굴기는 돌보는 사람이 있다고 본다.
# 2x2 채굴기의 산출 타일은 중심에서 1.5타일 안쪽이라 넉넉하게 잡았다.
CHEST_REACH = 2.5


# 한 번 걸어가서 몇 대까지 손볼 것인가. 너무 크게 묶으면 한 사람이 오래
# 붙들려 있고, 너무 작게 묶으면 같은 구역을 여러 번 왕복한다.
CLUSTER_REACH = 14


CLUSTER_MAX = 6


# 싣는 곳과 붓는 곳이 이보다 멀면 한 사람의 일이 아니다. 실제로 200타일
# 떨어진 상자에서 실어 오라는 계획이 나왔고, 길을 못 찾아 다섯 번 헤매다
# 빈손으로 「no coal to insert」로 끝났다.
HAUL_REACH = 120.0


# 제련 블록의 모양.
#
# 고수들이 쓰는 꼴은 «화로 두 줄이 마주보고, 가운데로 판금을 뽑는» 것이다
# (factorio wiki: Tutorial:Main bus / alt-f4.blog #56). 흩뿌리지 않는 이유는
# 보기 좋아서가 아니라, 나중에 벨트 한 줄로 전부 먹이고 전부 거둘 수 있어서다.
#
#   pitch 3   돌화로는 2x2 다. 3으로 두면 나중에 3x3 전기로로 «그 자리에서»
#             바꿀 수 있다. 붙여 놓으면 교체할 때 줄을 통째로 걷어야 한다.
#   row 12    한 줄에 열두 대. 두 줄이 한 블록(24대)이다.
#   aisle 5   마주보는 두 줄 사이. 가운데로 판금 벨트가 지나갈 길.
#   gap 9     블록과 블록 사이 통로. 사람이 지나다니고 벨트가 가로지른다.
#
# 예순 대를 놓으면 가로 36타일, 세로 42타일 안에 전부 들어간다.
FURNACE_PITCH = 3
FURNACE_ROW = 12
FURNACE_AISLE = 5
FURNACE_GAP = 9


# 화로는 돌 5개라 싸다. 상한이 8이었던 것이 259분 뒤의 상태를 만들었다 -
# 채굴기 194대가 화로 21대를 바라보고 있었고(정상 비율의 9.2배), 그래서
# 채굴기 101대가 «내놓을 데가 없어» 서 있는 옆에서 화로 21대가 굶었다.
# 캐는 능력이 모자란 적은 한 번도 없었다. 녹이는 능력이 모자랐다.
MAX_FURNACES = 60


# 버너 채굴기 0.25 광석/초, 돌 화로는 철광석 하나에 3.2초라 0.3125 광석/초.
# 화로가 살짝 빠르므로 «직결»이면 1 : 1 이다 - 드릴이 병목이라 화로를 더
# 붙여봐야 논다. 5:4 는 상자를 사이에 둘 때의 비율이고, 우리는 드릴을 화로에
# 바로 붙인다.
FURNACES_PER_DRILL = 1.0


# 보일러 60 증기/초 : 증기기관 30 증기/초. 기관을 하나만 붙이면 보일러가
# 만든 증기의 절반을 버리면서 석탄은 전부 태운다.
ENGINES_PER_BOILER = 2


# 보일러는 0.45/s 로 태운다. 20개면 44초다.
BOILER_FUEL = 20


# small-electric-pole 은 7.5타일까지 배선이 닿는다. 여유를 두고 7로 잡는다.
# 소형 전봇대. 목재 하나와 구리선 둘 - 판금 하나보다 싸다.
POLE = "small-electric-pole"
POLE_REACH = 7


# 전봇대 한 개는 나무 1 + 구리선 2. 랩 한 대는 전자회로 10 + 기어 10 +
# 벨트 4 라, 118타일을 잇는 전봇대 열일곱 개가 랩 한 대보다 싸다.
# 실제로 «랩을 못 만들었습니다»에서 막혀 전력이 서지 못했다.
# 이보다 멀면 전선을 끄는 대신 소비하는 쪽을 발전소 옆으로 옮긴다.
# 118타일에 전봇대 18개는 구리판 18장과 나무 18개다. 랩 하나가 훨씬 싸고,
# 사람도 그렇게 한다 - 전기는 길게 끌지 않고 공장을 전기 쪽으로 붙인다.
MAX_POLE_RUN = 8


# 버너 드릴 5대가 돌 화로 4대를 채운다. 뒤집으면 화로 4대에 드릴 5대.
DRILLS_PER_FURNACE = 1.0


# 버너 드릴은 석탄을 손으로 넣어줘야 한다. 돌볼 수 있는 것보다 많이 지으면
# 멈춘 기계만 늘어난다 - 한 사람이 셋까지.
DRILLS_PER_AGENT = 6


# 한 번 걸어가서 몇 대를 세우는가. 한 대씩 세우러 다니면 걷는 시간이 짓는
# 시간보다 길다 - 사용자가 «1개만 건설한다»고 지적한 것이 이것이다.
DRILLS_PER_TRIP = 4


# 화로가 다 녹이기 전에 꺼내러 가면 광석은 화로 안에 남고 손은 빈 채로
# 돌아온다. 게임이 알려준 시간에 여유를 더해서 기다린다.
SMELT_MARGIN = 8.0


# 화로 하나에 이만큼 쌓였으면 거두러 간다. 두세 개씩 집으러 왔다 갔다 하면
# 걷는 시간이 녹이는 시간보다 길어진다.
HARVEST_MIN = 10


# 화로가 받아주는 것들. 노는 화로에 무엇을 넣을지 고를 때 쓴다.
SMELTABLE = ("iron-ore", "copper-ore", "stone")


# 채굴기 출구에 화로를 바로 붙일 만한 광석. 채굴기가 앞 칸에 떨구면 그대로
# 제련이 시작되므로 사람이 퍼 나를 일이 없어진다. 돌은 화로에 넣을 일이
# 드물어 상자로 받는다.
SMELTED_BY_FURNACE = ("iron-ore", "copper-ore")


# 연구가 걸려 있는지 보는 주기. 매 틱 물어볼 일은 아니지만, 비어 있는 채로
# 오래 두면 랩이 그만큼 논다.
# 조립기 1호 한 대가 만드는 과학팩 0.1개/초 = 랩 한 대가 먹는 0.1개/초.
# 정확히 한 대가 한 대를 채운다. 초반에 이보다 깔끔한 비율은 없다.
SCIENCE_FEED = 40


RESEARCH_CHECK = 20.0


# 둥지를 살피는 주기와, 이 거리 안이면 알리는 기준. 공해가 퍼지는 속도에
# 비하면 30초는 충분히 촘촘하다.
THREAT_CHECK = 30.0


NEST_ALARM = 200


# 미리 세워둘 터렛 수와 한 대에 넣을 탄약. 습격이 온 다음에 짓기 시작하면
# 이미 늦다 - 터렛은 낭비가 아니라 대비다.
# 방어선 한 면이 백육십 타일이고 터렛 사거리가 18이다. 넷으로는
# 그 면의 4분의 1도 못 덮는다. 습격에 화로 8대와 요원 하나를
# 잃고 나서 올린 값이다.
#
# 이제 이것은 «게임이 답을 안 줄 때만» 쓰는 대비값이다. 몇 대가 있어야
# 하는지는 공해 여유가 정하고, 그 계산은 defence.lua 에 있다(`want`).
# 두 곳이 같은 것을 따로 세는 한 언제든 다시 어긋난다 - 실제로 어긋나
# 있었다. 게임이 12를 요구하는데 여기가 10에서 멈춰 빚 2가 영원히 남았고,
# 그 빚이 공장 성장과 기술 사다리를 통째로 묶었다.
TURRET_TARGET = 10


TURRET_AMMO = 10


# 기지 중심에서 터렛까지. 공해가 나가는 쪽이 습격이 들어오는 쪽이다.
TURRET_RING = 18


# 연구는 이 순서로 고른다. 방어가 과학팩보다 뒤로 밀리면, 둥지가 가까워진
# 다음에야 터렛을 만들기 시작한다.
# 실측(699분째): 「military」는 기관단총만 연다. 터렛은 «gun-turret» 이라는
# 별개의 연구고, 벽도 «stone-wall» 로 따로 있다. 방어를 military 로 알고
# 있어서, 그 연구가 끝난 뒤에도 터렛 레시피는 잠긴 채였다 - 그리고 바로
# 그때 바이터 열두 마리가 왔다.
# 총을 여는 연구. 「무장했는가」를 묻는 자리가 여럿이라 이름을 준다.
TURRET_TECH = "gun-turret"

RESEARCH_ORDER = (TURRET_TECH, "stone-wall", "military",
                  "automation", "logistics", "electric-mining-drill",
                  "steel-processing", "logistic-science-pack")


# 같은 사람이 같은 말을 이 시간 안에 되풀이하면 삼킨다.
ECHO_QUIET = 60.0


# 규칙이 할 일을 못 찾았을 때만 모델에게 묻는다. 이 간격 안에 두 번 묻지
# 않는다 - 막혀 있는 상태는 몇 초 만에 바뀌지 않는다.
# 배차 주기. 매 틱 무리 전체를 훑으면 RCON 왕복이 아깝고, 너무 뜸하면
# 놀고 있는 사람이 생긴다.
DISPATCH_INTERVAL = 10.0


# 한 번에 한 사람이 받아 가는 석탄.
HAUL_BATCH = 40


# 가방에 이만큼 넘게 쌓이면 공용 창고에 넣는다. 각자 안고 다니면 필요한
# 사람에게 가지 않는다 - 옆 사람이 철광석 150개를 든 채로 «철광석이
# 필요합니다»라고 말하는 일이 생긴다.
KEEP_IN_HAND = 50

# 캔 광석은 가방에 두지 않는다.
#
# 사용자 지시: "채굴한 광석들은 왠만하면 운반할때아니고서야 인벤토리에
# 두고있지말것."
#
# 맞다. 가방 속 광석은 아무도 못 쓴다. 옆 사람이 철광석 150개를 든 채로
# «철광석이 필요합니다»라고 말하는 일이 실제로 있었다. 게다가 가방이 차면
# 그다음 take 가 통째로 실패한다.
#
# 연료(석탄)는 다르다. 손에 들고 있어야 기계에 넣어줄 수 있으므로 KEEP_IN_HAND
# 를 그대로 쓴다. 광석은 한 줌만 남기고 전부 창고로.
KEEP_ORE = 10
#
# 판금은 여기 없다. 사용자가 말한 것은 «채굴한 광석»이지 판금이 아니고,
# 둘은 쓰임이 다르다 - 광석은 화로에 들어가면 그만이지만 판금은 손으로
# 무엇을 만드는 재료다. 판금을 열 개만 남기고 다 부으면 can_make 가 언제나
# 거짓이 되고, 그러면 사슬을 한 칸도 못 오른다.
#
# 실제로 그렇게 됐다(2026-09-18): 창고에 철판 3,979 구리판 7,774 를 쌓아두고
# 조립기 0대, 과학팩 0개, 랩 셋은 전부 「과학팩 없음」이었다. 채굴기는
# 161대까지 늘었다 - 만들 수 없으니 캐기만 한 것이다.
LOOSE_ORE = ("iron-ore", "copper-ore", "stone")


DEPOT_MIN = 20


# 연료 떨어진 기계가 이만큼이면 «굶고 있다»고 본다. 그럴 때는 광석보다
# 석탄이 먼저다.
STARVING = 4


# 같은 것을 이만큼이 부탁하면 그건 부탁이 아니라 부족이다. 없는 사람끼리
# 주고받아 봐야 아무것도 안 채워진다 - 늘려야 한다.
SHORTAGE_VOICES = 3


# 한 번에 세울 석탄 자급쌍 수. 사용자가 말한 「네 개를 서로 마주보게」가
# 이 쌍 두 벌이다. 쌍끼리는 서로 먹이지 않으므로 넷이 한 덩어리가 아니라
# 둘씩 두 덩어리다 - 그래도 손이 갈 일은 똑같이 없다.
COAL_PAIRS = 2


# 한 번에 세울 급유 장치 수, 그리고 상자에 부어둘 석탄.
# 버너 인서터는 자기가 나르는 게 연료일 때만 자급한다. 석탄을 나르는
# 인서터는 영원히 돌고, 광석을 나르는 인서터는 손이 계속 간다 - 그래서
# 급유만 자동화하고 광석은 아직 손으로 옮긴다.
RIGS_PER_TRIP = 3


RIG_COAL = 50


IDLE_ASK_QUIET = 150.0


# 그럴 때 쓰는 모델. 반장이 지시를 쪼갤 때와는 판단의 무게가 다르고,
# 자주 일어나는 일이라 싼 쪽이 맞다.
IDLE_MODEL = "haiku"


# 기지에서 이만큼 밖에 홀로 선 우리 건물은 「도망간 것」으로 본다.
#
# 실측(2026-09-18): 화로 63대 중 43대가 60타일 밖이었고, 가장 먼 것이
# 242타일이었다. 그쪽에는 광석이 닿지 않아 영원히 놀면서, 대신 사람을
# 그쪽으로 끌고 간다 - 요원 하나가 (322,13)에 서서 기지를 보지도 못했다.
STRAY_FAR = 60

# 사람이 기지에서 이만큼 밖에 있으면, 할 일을 찾기 전에 돌아온다.
#
# 에이전트 중심 조회는 전부 반경 200 안만 본다. 그 밖에 서 있으면 기지가
# «안 보이고», 안 보이니 할 일이 없고, 할 일이 없으니 돌아올 이유도 못 찾는다.
HOME_REACH = 120


# 석탄 자급쌍의 연료칸이 이만큼 차면 「꽉 찼다」로 본다 (한 칸은 50개).
#
# 마주보는 두 대는 캔 석탄을 서로의 연료칸에 넣는다. 출력 상자가 없으니
# 갈 곳이 연료칸뿐이고, 그 한 칸이 차면 채굴기가 선다. 즉 이 쌍은 필요할 때만
# 꺼내 쓰는 창고가 아니라, **주기적으로 비워줘야 굴러가는** 장치다.
#
# 실측(2026-09-18): 석탄 위 채굴기 48대가 전부 50/50 으로 멈춰 있었다.
WELL_FULL = 45


# 캐는 구역에서 캔 것을 받아두는 상자.
#
# 나무 상자는 목재 둘, 철 상자는 철판 여덟이다. 초반에 철판은 사슬의 목이고
# 나무는 발에 차인다. 버퍼로 쓸 상자를 철로 만드는 것은 사다리를 스스로
# 끊는 일이다. 칸이 16개뿐이지만 어차피 주기적으로 비운다.
MINE_CHEST = "wooden-chest"

# 캐는 구역의 상자가 이만큼 차면 제련 구역으로 옮긴다.
HAUL_WHEN = 100


# 조립 구역. 조립기도 랩도 3x3 이라 4칸 간격이면 한 칸씩 남는다 - 그 한 칸이
# 인서터와 벨트가 지나갈 길이다. 한 줄에 여섯 대면 24타일, 구역 폭에 맞는다.
CRAFT_PITCH = 4
CRAFT_ROW = 6


# 가방에 빈 칸이 이만큼도 안 남으면, 다른 무엇보다 먼저 비운다.
#
# 가방이 차면 캐는 일도 걷어내는 일도 줍는 일도 전부 «조용히» 실패한다.
# 실측(2026-09-18): 여덟 캐릭터 중 여섯이 빈 칸 0 이었고, 한 명은 demolish 를
# 띄운 채 가만히 서 있었다. mine_entity 가 false 를 돌려주는데 아무도 그
# 값을 안 봤기 때문이다.
BAG_ROOM = 6

# 가방에 «둘 만큼». 한 번 나가서 일하는 데 드는 양이다.
#
# 사용자: "캐릭터들 인벤토리에 남는 자원들이 많은데 왜 물류창고에 안넣지?"
#
# 실측(새 판 125분째):
#
#     다섯이 들고 있는 것   석탄 4,831  구리판 1,255  철판 1,050  돌 419
#     공용 창고             돌 35  구리광석 89  철광석 36  석탄 40
#     빈 칸                 27~60개
#
# 창고에 붓는 일감이 「가방이 거의 꽉 찼을 때」(빈 칸 6 이하)만 떴다.
# 아무도 그만큼 차지 않으니 자원의 99%가 개인 가방에 잠겨 있었다. 가방에
# 있는 것은 그 사람만 쓴다 - 옆 사람이 철판 1,050장 옆에서 "철판이
# 모자랍니다"라고 말한다.
BAG_KEEP = {
    "coal": 200,            # 화로와 채굴기를 먹이며 한 바퀴 도는 데 넉넉
    "iron-plate": 200,      # 기어·상자·벨트를 만들며 도는 데 넉넉
    "copper-plate": 200,
    "stone": 100,           # 화로 스무 대분
    "iron-ore": 100,
    "copper-ore": 100,
}

# 붓는 문턱은 두는 양보다 «높아야» 한다.
#
# 같으면 붓고 나서 바로 다시 꺼내 온다. 오늘 채굴기 자리에서 이미 치른
# 값이다 - 세우는 문턱과 걷는 문턱이 같아서 세우면 걷고 걷으면 세웠다.
BAG_SPILL = 2.0


# 버너 시대에 몇 대면 되는가 — 숙련자들의 권장치.
#
# 리서치(2026-09-18, 포럼 경험담 MED): 철 채굴기+화로 약 20쌍, 구리 5~10쌍,
# 석탄 10대 남짓, 돌 4대. 합쳐 마흔 대 안팎이다.
#
# 우리는 161대를 세웠다. 권장치의 네 배에서 여덟 배다. 그러고도 빨간 과학팩이
# 0개였다 - 너비만 늘리고 사슬은 한 칸도 못 올랐다. 숙련자들이 한목소리로
# 경고하는 것이 정확히 그것이다: "체인 앞이 광석 부족인데 뒤만 늘리기 금지",
# "과학팩이 밀려 쌓일 때만 랩 추가".
#
# 그래서 버너 시대에는 상한을 둔다. 전기 채굴기로 넘어가면(속도 두 배, 범위
# 5x5) 이 상한은 의미가 없어지므로 그때 푼다.
BURNER_DRILLS = 40

# Automation 연구에 필요한 빨간 과학팩. 게임 공식값이고, 손으로 만들어도 된다.
# 이 열 개가 그다음 전부의 문이다 - 로지스틱도 전기 채굴기도 터렛도 그 뒤다.
FIRST_PACKS = 10

# 한 흐름을 동시에 몇 사람이 깔 것인가. 넷째부터는 서로 철판을 굶긴다.
LINE_HANDS = 3

# 공해만 내는 채굴기가 이만큼 넘게 있으면 걷기 시작한다. 두세 대가 잠깐
# 막힌 것은 고장이 아니라 그냥 파도다.
SMOKING_FLOOR = 12

# 채굴기와 화로가 서로를 기다리지 않도록 한 번에 앞서갈 수 있는 걸음.
# 비율은 «방향»이지 «상한»이 아니다 - 상한은 버너 한계·공해 여유·자리표가
# 따로 지킨다.
GROW_STEP = 2

# 한 번에 몇 칸까지 둘러보고, 몇 칸까지 걷어올 것인가. 벨트 한 스택이
# 백 개라 스물은 가방에 넉넉하다.
LOOSE_LOOK = 120
LOOSE_BATCH = 20
# 길 밖 벨트가 이만큼 넘게 널려 있으면, 새로 만들기 전에 걷는 일이 먼저다.
LOOSE_FLOOD = 30

# 길을 한 번에 몇 칸씩 맡을 것인가. 아래는 가장 적게 맡는 양이고, 위는
# 손에 벨트가 많아도 이 이상은 안 맡는다 - 한 사람이 다 쥐면 나머지가 논다.
LINE_CHUNK = 20
LINE_CHUNK_MAX = 60

# 석탄 채굴기를 몇 대까지는 상한을 넘겨서라도 세울 것인가.
#
# 버너 채굴기 하나가 초당 0.25개를 캐고 태우는 것은 0.0375개다. 제 몫의
# 일곱 배를 남기므로 넷이면 스물여덟 대를 먹인다. 석탄은 모자라면 느려지는
# 것이 아니라 «되돌아올 수 없다» - 연료가 끊기면 석탄 채굴기도 멈춘다.
COAL_RIGS = 4

# 석탄 채굴기를 «세어서» 정하는 데 드는 값. 상수 넷은 판마다 다른 숫자였다.
#
# 게임이 주는 값이다:
#   버너 채굴기  150kW,  초당 0.25개를 캔다
#   돌 화로       90kW
#   석탄 한 개    4MJ
#
# 그래서 태우는 속도는 채굴기 0.0375/s, 화로 0.0225/s 다. 석탄 채굴기도
# 제가 태우니 남기는 것은 0.25 - 0.0375 = 0.2125/s 다.
COAL_PER_SECOND = 0.25            # 버너 채굴기가 캐는 속도
DRILL_BURN_PER_SECOND = 0.0375    # 버너 채굴기가 태우는 속도
FURNACE_BURN_PER_SECOND = 0.0225  # 돌 화로가 태우는 속도

# 적이 이보다 가까우면 일을 놓고 물러난다.
#
# 요원은 총이 없다. 실측으로 일흔네 번 죽는 동안 바이터는 한 마리도 못
# 잡았다 - 싸운 것이 아니라 서 있다가 물린 것이다. 그리고 죽으면 들고
# 있던 것을 전부 흘린다.
DANGER_NEAR = 40
# 한 마리 늘 때마다 물러나는 거리를 이만큼 더 준다. 스물일곱 마리가
# 쫓을 때와 둘이 쫓을 때는 달아날 수 있는 거리가 다르다. 스무 마리에서
# 멈춘다 - 그보다 많으면 어차피 거리가 아니라 운의 문제다.
DANGER_PER_HEAD = 3

# 공해가 둥지까지 이만큼 남으면 방어가 급한 일이 된다.
#
# 터렛 한 대를 세우고 탄약을 채우는 데 몇 분이 든다. 닿은 뒤에 시작하면
# 세우는 동안 습격이 온다. 지난 판은 여유 13타일에서 터렛 2대, 탄약 0
# 이었고 화로 11대와 요원 74번을 잃었다.
DEFEND_WHEN = 60

# 아무도 일감을 못 받았을 때 다시 볼 때까지의 시간(초).
#
# 사람이 놀고 있는 것보다 비싼 것은 없다. 다만 0 으로 두면 배차가 매
# 순찰 돌아 게임이 무거워지므로, 한 번 숨 쉴 만큼만 둔다.
DISPATCH_IDLE = 2.0

# 기지가 뚫렸을 때 떼의 반대쪽으로 몇 타일 물러나는가.
#
# 작은 바이터가 사람보다 빠르므로 「멀리 갈수록 안전」이 아니다. 떼의
# 추격 사거리를 벗어나되 돌아올 수 있는 거리여야 한다. 터렛이 서면
# 이 값은 뜻을 잃는다 - 그때는 물러날 일이 없다.
RETREAT_SPAN = 60

# 「이미 충분히 쌓였다」고 볼 광석의 양(무리 전체 합계).
#
# 돌화로 한 대는 광석을 3.2초에 하나 녹인다 - 분당 열여덟 개다. 백 개면
# 한 대가 여섯 분을 꼬박 일해야 하는 양이고, 그만큼 쌓여 있는데도 판금이
# 안 나온다면 모자란 것은 광석이 아니다.
PILED_UP = 100

# 위협을 얼마나 멀리까지 살피는가.
#
# 이 값은 도망 기준보다 «커야 한다». 작으면 기준이 반경에 갇힌다 -
# 여든 타일만 살피면서 백 타일 기준으로 판정하면, 그 사이 스무 타일에
# 있는 적은 규칙상 도망칠 거리인데 보이지가 않는다. 실제로 그래서 둘이
# 죽었다.
#
# 그리고 총이 없을 때는 이 값이 «곧 도망 기준»이다. 보이면 물러난다.
DANGER_LOOK = 120

# 「넓게 캐도 되는가」를 가르는 공해 여유(타일).
#
# 버너 채굴기 마흔 대는 분당 사백의 공해다. 둥지까지 여유가 이보다 적으면
# 넓게 캐는 것이 총을 앞당기는 게 아니라 습격을 앞당긴다.
NOISE_ROOM = 80

# --- 각자 생각하는 머리 ------------------------------------------------
#
# 사용자: "각 캐릭터들을 1개씩 sonnet5모델의 CLI를 달아서 직접 생각하고
# 행동하도록"
#
# 빈 값이면 머리를 안 단다. 규칙만으로도 공장은 돌아가고, CLI 가 없는
# 자리에서 무리가 통째로 멈추면 안 된다.
MIND_MODEL = "sonnet"
# 한 사람이 다시 생각하기까지(초).
#
# 45초로 뒀더니 기계가 무릎을 꿇었다. 실측:
#
#     node 76개 / 47 GB     남은 RAM 3.2 GB (총 61.6 GB)
#     그중 50개가 «지금 돌고 있는» 머리 호출, 34 GB
#
# `claude -p` 한 번이 1~4 GB 를 쓰고, 한 번에 45초보다 오래 걸린다.
# 다섯이 45초마다 부르면 끝나기 전에 다음 것이 시작되어 겹겹이 쌓인다.
# 새는 것이 아니라 «양»이다 - 고아 프로세스는 한 개도 없었다.
MIND_EVERY = 120.0
# 동시에 생각하는 사람 수.
#
# 이것이 진짜 제동이다. 주기만 늘리면 다섯이 같은 순간에 시작하는 것을
# 못 막는다 - 손이 비는 때가 서로 비슷하기 때문이다.
#
# 하나로 둔다. 다섯이 돌아가며 생각해도 각자 열 분에 한 번꼴이고, 그
# 사이에는 규칙이 고른다. 규칙은 공짜고 머리는 기가바이트다.
MIND_AT_ONCE = 1
# 한 번 생각에 이만큼 넘으면 버린다. 답이 늦으면 이미 상황이 달라져 있다.
MIND_TIMEOUT = 60.0


# -- 소비성 재료 ------------------------------------------------------------
#
# 사용자: "항상 연료랑 탄약같이 소비성재료들은 넉넉하게 넣어둘 것."
#
# 떨어진 «뒤에» 채우면 그 사이 멈춰 있던 시간이 그대로 손해다. 16회차
# 실측: 줄 끝의 버너 인서터 하나가 굶어 채굴기 서른 대가 벨트를 꽉 채운
# 채 멈췄고, 아무것도 「실패」하지 않았으므로 아무도 몰랐다.
#
# 그래서 문턱을 둘로 나눈다 - «언제 가는가»(LOW)와 «얼마나 붓는가»(FULL).
# 하나로 쓰면 채우자마자 또 채우러 가거나, 빈 뒤에야 간다.
COAL_LOW = 20                     # 연료가 이 아래면 채우러 간다
COAL_FULL = 50                    # 버너 연료칸 한 스택
# 「넉넉하게」는 «마르지 않게»라는 뜻이지 «쌓아 두라»는 뜻이 아니다.
#
# 탄약 한 발은 철판 넷이고, 포탑 한 대는 철판 마흔이다. 한 대를 100발로
# 채우면 그 탄창에 철판 400장 - 포탑 «열 대» 값이 들어간다.
#
#     실측(21회차): 포탑 9대에 탄약 317발, 창고에 43발.
#                   그때 덮인 건물은 12/101 = 11% 였고,
#                   적은 120칸 안에 0마리, 가장 가까운 둥지가 999칸이었다.
#
# 쏜 탄약이 «한 발도 없는» 동안 철판 1,400장이 탄창으로 누워 있었고,
# 둘레의 89%는 열려 있었다. 위키(Enemies)가 말하듯 벌레는 열린 길로
# 걸어가므로, 포탑이 없는 곳에 탄약을 쌓는 것은 그 길을 막지 못한다.
#
#     마르지 않는 것이 목적이지 쌓이는 것이 목적이 아니다.
#
# 30발이면 지속 사격 30초고, LOW 15 에서 순찰이 이미 출발한다. 떨어진
# 뒤에 가는 일은 여전히 없다 - 바뀐 것은 「얼마나 붓는가」뿐이다.
AMMO_LOW = 15                     # 포탑 탄창이 이 아래면 채우러 간다
AMMO_FULL = 30                    # 한 대에 이만큼까지 채운다


# -- 출구는 몇 개여야 하는가 -------------------------------------------------
#
# 버너 인서터 하나가 초당 0.6개를 옮긴다. 버너 채굴기 하나가 초당 0.25개를
# 캔다. 그러니 «출구 하나에 채굴기 두 대»가 한계다.
#
# 실측(16회차 석탄밭): 채굴기 10대에 출구 하나.
#
#     벨트     칸마다 8개로 포화
#     채굴기   10대 중 6대 waiting_for_space_in_destination
#     실제 산출 0.6개/초    <- 캘 수 있는 2.5개/초의 4분의 1
#     상자     13개
#
# 온 맵이 석탄을 기다리고 있었다. 채굴기를 늘려도 소용이 없다 - 늘어난
# 것은 「기다리는 채굴기 수」뿐이다. 병목은 «문»이지 «광부»가 아니었다.
INSERTER_PER_SECOND = 0.6
DRILLS_PER_ARM = max(1, int(INSERTER_PER_SECOND / DRILL_PER_SECOND))

# 화로 «원료»칸. 연료만 보면 불은 붙어 있는데 아무것도 안 구워지는 화로가
# 줄줄이 선다. 사용자: "연료 / 재료 / 탄약 등등은 주기적으로 채우도록".
ORE_LOW = 15
ORE_FULL = 50

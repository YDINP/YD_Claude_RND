...
class MiningMixin:
    ...
    def automate(self, worker: Worker, ore: str | None) -> None:
        ...
        try:
            ...
            if not patch:
                # 아직 이 광맥에 채굴기가 한 대도 없다.
                #
                # zones 는 «채굴기가 선» 밭만 알려준다. 그래서 첫 대는
                # 영원히 자리표를 못 쓰고 옛 길로 아무 데나 선다. 그런데
                # «첫 대가 줄을 정한다» - 한 번 어긋나면 그 뒤가 전부
                # 어긋나고, 어긋난 것들이 자리표 자리를 막는다.
                #
                # 실측(새 판 78분째): 석탄 채굴기 18대가 서 있는데 자리표는
                # 「이미 선 것 0, 막힘 26, 남은 자리 2」라고 답했다. 열여덟
                # 대가 자리표 자리에 하나도 안 서 있었고, 떨구는 자리가
                # 여섯 줄로 흩어졌으며, 대부분이 «서로에게» 떨구고 있었다.
                # 석탄끼리 서로 먹이면 연료칸이 차고 둘 다 멈춘다.
                #
                # 자리표는 광맥에서 테두리를 «다시 재므로», 광맥 위의 점
                # 하나만 있으면 된다. 밭이 없으면 그 점을 준다.
                patch = {"left": spot["x"] - 1, "top": spot["y"] - 1,
                         "right": spot["x"] + 1, "bottom": spot["y"] + 1,
                         "ore": ore}
            if patch:
                try:
    ...
    def open_drill(self, worker: Worker, at: dict, quiet: bool = False) -> bool:
        ...
        57대가 이 상태였고, 바로 옆에서 화로 63대 중 60대가 광석이 없어 놀고
